...
class StDisk(Gtk.Grid):
	...
	def takas_diski_ata(self,widget,disk):
		disk = "{} swap {}G [SWAP] 0".format(disk[1],disk[3])
		print(disk)
	...
	def efi_diski_ata(self,widget,disk):
		...
		if tur == "fat32":
			tur = "vfat"
		disk = "{} {} {}M /boot/efi 0".format(disk[1],tur,self.gb_to_mb(disk[3]))
		print(disk)
	...
	def disk_secildi(self,widget):
		...
		if disk != None:
			...
			for bolum in self.diskler_liste[disk]["bölüm"]:
				...
				b = self.bayraklar_duzenle(bolum["bayraklar"])
				if "boot" in b and "esp" in b and self.oto_ata:
					bagnok = "/boot/efi"
					tur = bolum["tip"]
					if tur == "fat32":
						tur = "vfat"
					disk = "{} {} {}M /boot/efi 0".format(bolum["yol"],tur,self.gb_to_mb(bolum["boyut"]))
					self.ebeveyn.milis_ayarlari["uefi_disk"] = disk
					self.oto_ata = False
				self.disk_liste_store.append([icon,bolum["yol"],bolum["tip"],bolum["boyut"],bagnok,bolum["bayraklar"]])
				sayac += 1

	def gb_to_mb(self, boyut):
		boyut = boyut.replace(",",".")
		boyut = float(boyut)
		boyut = boyut * 1024
		print(boyut)
	...
	def bayraklar_duzenle(self,bayraklar):
		donecek = []
		b = bayraklar.split(",")
		for bayrak in b:
			if bayrak != "":
				bayrak = bayrak.replace(" ","")
				donecek.append(bayrak)
		return donecek
