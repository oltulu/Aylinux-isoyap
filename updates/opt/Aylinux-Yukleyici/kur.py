...
class MerkezPencere(Gtk.Window):
	...
	def ileri_basildi(self,widget):
		...
		if self.stack_secili == 3:
			...
			if type(self.milis_ayarlari["grub_kur"]) == str:
				a = self.milis_ayarlari["sistem_disk"].split()[0][:-1]
				if a != self.milis_ayarlari["grub_kur"]:
	...
	def baslik_ekle(self,stack):
		if self.stack_secili == 6:
			...
			if cevap == Gtk.ResponseType.OK:
				...
				self.ileri_dugme.set_sensitive(False)
				self.geri_dugme.set_sensitive(False)
				stack.terminal.komutlar.append("echo 'Kurulum Başlıyor'")
				stack.terminal.komutlar.append("milis-kur /tmp/milis_kur_ayar.txt text")
